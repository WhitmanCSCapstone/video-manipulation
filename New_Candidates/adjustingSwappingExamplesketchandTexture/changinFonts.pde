  ...
  void update(PGraphics p) {
    if (pauseToggle){
      ...
      if(redToggle){
        ...
        // --------------------
        // TEXT ALPHA CONTROL:
        float textAlphaControl = map(cc[3],0,127,0,255);
        // --------------------
        // SPEED CONTROL: how often to change the word (if (frameCount % speed == 0...)) 
        int speedControl = round(map(cc[1],0,127,30,2));  // Shouldn't be float b/c % 0
        // --------------------
        // TEXT BOX SIZE CONTROL: how much of the screen text should take up (in box)
        float boxSizeControl = map(cc[2],0,127,0.3,.9);
        // --------------------
