...
void setup() {
 ...
 // movie = new Movie(this, "multiply 1.mov");
    // movie.read();
 // movie.loop();
  ...
  String[] document = loadStrings("word_list.txt");   // Calls the text file from your data folder and loads it (by line breaks) into an array.
...
// SPEED CONTROL: how often to change the word (if (frameCount % speed == 0...)) 
  int speedControl = round(map(cc[17],0,127,60,2));  // Shouldn't be float b/c % 0
// --------------------
